...
| # | 动效 | 实现 |
| --- | --- | --- |
| 1 | 首屏：光球 + 标题淡入，单张牌背在屏幕中央浮现，并**原地极缓轻摆** | GSAP `fromTo`（标题）、`renderOrbs()` 内 `fromTo`（光球）、缩放 + 透明度 tween（牌）；轻摆 = 左右（周期 16.8s）与上下（周期 20.4s）两条 `yoyo` 补间错开周期叠加 |
| 1b | 单张牌背**跟随指针**微微转向（桌面鼠标移动 / 移动端手指拖动，松手回原位） | 每帧插值（`1-(1-k)^(dt·60)`，与帧率无关，无弹簧感），最大 6°；仅初始态生效，展开后停止 |
...
- **批次 E（已完成）**：单张牌漂浮收敛 + 指针跟随倾斜；标题炸开改为「淡出 + 26 颗星光（canvas 单层）」解决卡顿
